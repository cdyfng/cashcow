...
#reset
kill `ps -ef|grep 'website/bin/www'|grep -v 'grep'|awk '{print $2}'` 
proc_num
number=$?  
echo $number
if [ $number -eq 0 ]                                    # 判断进程是否存在  
then
    echo 'no cashcow www' >> $file_name      # 将新进程号和重启时间记录  
    ...
    nohup /usr/local/bin/node /root/work/cashcow/website/bin/www >> /root/work/cashcow/website/www.log&
